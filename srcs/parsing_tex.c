...
/*   Updated: 2025/12/28 22:46:25 by ksupinsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_texture_path(t_game *game, char *tokens)
{
	...
	if (!tokens)
		error_exit(game, "Texture path missing\n");
...
{
	if (game->tex.no)
		error_exit(game, "Duplicate NO texture definition\n");
	else if (!tokens[1])
		error_exit(game, "NO texture path missing\n");
	else
	{
...
{
	if (game->tex.so)
		error_exit(game, "Duplicate SO texture definition\n");
	else if (!tokens[1])
		error_exit(game, "SO texture path missing\n");
	else
	{
...
{
	if (game->tex.we)
		error_exit(game, "Duplicate WE texture definition\n");
	else if (!tokens[1])
		error_exit(game, "WE texture path missing\n");
	else
	{
...
{
	if (game->tex.ea)
		error_exit(game, "Duplicate EA texture definition\n");
	else if (!tokens[1])
		error_exit(game, "EA texture path missing\n");
	else
	{
